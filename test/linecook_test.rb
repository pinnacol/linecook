...
Contents of a template file.
  ...
  def test_init_regenerates_cookbook_on_force
    example_readme = test_path('example/README')
  ...
  def test_init_does_not_allow_force_for_parent_dirs_or_current_dir
    example_dir = test_path('parent/current')
    FileUtils.mkdir_p(example_dir)
    ...
    assert_equal [example_dir], Dir.glob(test_path('parent/*'))
    assert_equal [], Dir.glob(test_path('parent/current/*'))
  end
end